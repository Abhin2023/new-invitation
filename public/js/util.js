...
export const util = (() => {
    ...
    const countDownDate = () => {
        const until = document.getElementById('count-down-1').getAttribute('data-time').replace(' ', 'T');
    ...
    const countDownDate2 = () => {
        const until = document.getElementById('count-down-2').getAttribute('data-time').replace(' ', 'T');
        const count = (new Date(until)).getTime();

        setInterval(() => {
            const distance = Math.abs(count - (new Date()).getTime());

            document.getElementById('day-2').innerText = Math.floor(distance / (1000 * 60 * 60 * 24));
            document.getElementById('hour-2').innerText = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
            document.getElementById('minute-2').innerText = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
            document.getElementById('second-2').innerText = Math.floor((distance % (1000 * 60)) / 1000);
        }, 1000);
    };
